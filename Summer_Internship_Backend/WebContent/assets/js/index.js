...
init();
  ...
  function addTableData(){
    ...
    let keys = ["name_customer", "cust_number", "invoice_id", "total_open_amount", "due_in_date_1", "payment_predicted", "notes"];
    let tbody = "<tbody>"
    list.forEach((row) => {
...
const add = () => {
  ...
  fetch(`/H2HBABBA2483/add?name_customer=${name_customer}&cust_number=${cust_number}&invoice_id=${invoice_id}&total_open_amount=${total_open_amount}&due_in_date_1=${due_in_date_1}&notes=${notes}`,
  {
    method: 'POST'
  }
  ).then(function(){
...
console.log(total_open_amount, notes);
    ...
    fetch(`/H2HBABBA2483/edit?invoice_id=${invoice_id}&total_open_amount=${total_open_amount}&notes=${notes}`, {
      method: 'POST'
    }
    ).then(function(){
      selectedRows = [];
...
const deleteData = () => {
  selectedRows.forEach((ele) => {
     ...
     fetch(`/H2HBABBA2483/delete?invoice_id=${invoice_id}`,
     {
      method: 'POST'
    }
     ).then(function(){
